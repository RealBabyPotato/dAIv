...
class GPTAPI {
    ...
    public static void main(String[] args) throws InterruptedException, NameNotFoundException {
        ...
        System.out.println(sendAndReceive(j, "remind me to cook bread tomorrow at 5pm"));
    }
    ...
    public static String sendAndReceive(User user, String message) {
        try {
            ...
            if(response.contains(".reminder{")){
                ...
                String timeSinceEpoch = response.substring(response.indexOf(".reminder{") + 10, response.indexOf('}'));
                ...
                response = response.substring(0, response.indexOf(".reminder{")) + response.substring(response.indexOf("|") + 1);
            }
    ...
    private static String createRun(String threadId, User user) {
        ...
        String requestBody = "{"
                ...
                + "\"instructions\": \"[" + Event.timeFormat.format(new Date()) + " " + Event.dateFormat.format(new Date()) + "]" + INSTRUCTIONS +  " The user's name is " + user.getUserName() +  " and this is their prompt to you: \""
                + "}";
